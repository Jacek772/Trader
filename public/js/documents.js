...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const documentsdefinitionsOptionsData = documentsdefinitionsRes.data.documentsdefinitions.map(x => ({
        text: x.symbol,
    ...
    filtersPanel.addItem(new FilterPanelItem("Definition", ["definition"], FilterPanelItem.types.SELECT_WITH_EMPTY, documentsdefinitionsOptionsData))
    ...
    btnOpenForm.addEventListener("click", async () => {
        ...
        dataForm.setDataObject(checkedData[0])
        dataForm.setTitle(checkedData[0].number)
        dataForm.setOnSave(async (formDataObject) => dataFormSaveUpdate(modal, dataForm, formDataObject))
        dataForm.show()
    })
